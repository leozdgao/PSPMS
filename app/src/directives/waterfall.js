...
.factory('wfInstance', [function(){
	...
	Waterfall.prototype.initColumns = function(num) {
	    if(num > 0) {
	    	var that = this;
	    	// remove old wf-column
	    	[].forEach.call(this.container.querySelectorAll('.wf-column'), function(elem) {
	    		that.container.removeChild(elem);
	    	});
...
.directive('waterfall', ['wfInstance', function(wfInstance) {
	// Runs during compile
	return {
		// name: '',
		// priority: 1,
		...
		link: function($scope, iElm, iAttrs, controller) {
			...
			var boxes = container.querySelectorAll('wf-box');
			$scope.$watch('source', function(newVal, oldVal) {
				if(newVal !== oldVal && newVal.length > 0) {
					// compose waterfall
					// console.log(container.children);
					new wfInstance({ minBoxWidth: 300 });
				}
			});
		}
	};
